...
plotCases <- jpCasesObs %>%
  ...
  labs(title = "Percent of All Cases Observed by JP Court")
...
plotReason <- evicReason %>%
  ...
  coord_polar(theta = "y") +
  facet_grid(~jpCourt) +
  labs(title = "Reason for Eviction Case Filing by Court") +
  theme_cpal_map(scale = "discrete") +
  facet_grid(~jpCourt) +
  scale_fill_manual(values = palette_cpal_main)

cpal_plot(plotReason, 
          cpal_logo_text(), 
          ncol = 1,
          heights = c(30, 1))
...
cpal_plot(cpal_title(string = "Landlords or their representatives almost always appeared for the hearing, but tenants appeared less than half the time.",
                     width = 75),
          plotAppear, 
          cpal_logo_text(), 
          ncol = 1,
...
cpal_plot(cpal_title(string = "Tenants rarely had legal representation.",
                     width = 75),
          plotDef, 
          cpal_logo_text(), 
          ncol = 1,
...
plotOutcome <- caseOutcome %>%
  mutate(outcomeParty = factor(outcomeParty, levels = c("Abated", "Dismissed", "Ruled in Favor of Landlord", "Ruled in Favor of Tenant", "Other", "Unknown"))) %>%
...
cpal_plot(cpal_title(string = "Across all observed cases, the judge ruled in favor of the landlord in XX% of cases and in favor of the tenant in YY% of cases.",
                     width = 75),
          plotOutcome, 
          cpal_logo_text(), 
          ncol = 1,
...
plotOutAp <- outcomeApp %>%
  ...
  geom_bar(stat="identity") +
#  geom_text(aes(label = paste0(round(perOutcome*100, digits = 0), "%")),
#                fontface = "bold") +
  theme_cpal_print() +
  scale_y_continuous(labels = scales::percent) +
  coord_flip() +
...
cpal_plot(cpal_title(string = "Case outcomes when tenant/defendant appeared versus when tenant/defendant did not
...
cpal_plot(cpal_title(string = "Case outcomes when tenant/defendant had legal representation versus when

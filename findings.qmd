...
evicReason <- courtobs %>%
  ...
  summarize(totCases = n()) %>%
  mutate(perCases = totCases/sum(totCases)) %>%
  ungroup(.)
```
...
jpCasesObs %>%
  filter(!is.na(jpCourt)) %>%
  ggplot(aes(x = court, y = perAllCases)) +
...
```{r, Cases by Reason}
evicReason %>%
  ggplot(aes(fill=evicClean, y=jpCourt, x=totCases)) + 
...
> *In fall 2021 in JP 1-1, tenants had legal representation 8% of the time.
...
> *In fall 2021 in JP 1-1, a Notice to Vacate was confirmed 40% of the time.

...
library(DT)
jpCasesObs %>%
  select(court, obsCases, totEvic, perAllCases) %>%
  datatable(options = list(paging = FALSE,
                           searching = FALSE,
                           info = FALSE
                           ),
            rownames = FALSE,
            colnames = c("JP Court", "Observed Cases", "Evictions Filed", "Percent of Cases")) %>%
  formatPercentage("perAllCases", 1) %>%
            formatCurrency(c("obsCases", "totEvic"),
                           interval = 3,
                           mark = ",",
                           currency = "",
                           digits = 0)
